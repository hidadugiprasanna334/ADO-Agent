class AzureFoundryChat {
    ...
    async initializeAzureClient() {
        try {
            // For browser environment, we'll use fetch-based approach
            // Initialize connection to Azure AI Projects
    ...
    async testConnection() {
        try {
            // Test the connection by trying to retrieve the assistant
            const response = await this.makeApiCall('GET', `/assistants/${this.config.agentId}`);
            console.log('Connection test successful:', response);
            return response;
        } catch (error) {
            console.error('Connection test failed:', error);
            // Don't fail initialization if assistant check fails
            console.log('Proceeding without assistant verification...');
            return null;
        }
    }
    ...
    async makeApiCall(method, endpoint, data = null) {
        ...
        const url = `/api/azure${endpoint}`;
        
        console.log(`Making ${method} request to proxy: ${url}`);
